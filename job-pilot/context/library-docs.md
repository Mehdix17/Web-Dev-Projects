...
// Always use and flush/shutdown in route handlers / actions
const posthog = getPostHogClient();
posthog.capture({
  distinctId: userId,
  event: "sign_out_completed",
});
await posthog.flush(); // or await posthog.shutdown()
```

**Rules:**

- Always call `await posthog.flush()` or `await posthog.shutdown()` in server-side functions/actions — events are lost without it
- `flushAt: 1` and `flushInterval: 0` always set on server client
- Event names must match exactly the list in `context/project-overview.md`
- Always include `distinctId` when capturing server-side events
- Call `posthog.identify(userId)` after login on client side (or render `<PostHogIdentify>`)
