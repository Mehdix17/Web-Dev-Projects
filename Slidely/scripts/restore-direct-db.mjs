...
async function restoreWorks() {
  try {
    ...
    if (existing.length > 0) {
      console.log("⚠️  TRIPLY already exists in database, skipping...\n");
    } else {
      ...
      await sql`
        INSERT INTO works (
          ...
          featured, thumbnail, pdf_url, summary, slides
        ) VALUES (
          ${triply.slug},
          ${triply.title},
          ${triply.category},
    ...
    const allWorks =
      await sql`SELECT slug, title, slides FROM works ORDER BY year DESC, slug ASC`;
    ...
    if (triplyRestored) {
      ...
      console.log("✈️  The gallery is now complete!\n");
    }
  } catch (error) {
